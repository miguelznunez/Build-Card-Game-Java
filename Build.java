...
public class Build
{
    ...
    computerCardsLeft = NUM_CARDS_PER_HAND, humanCardsLeft = NUM_CARDS_PER_HAND;
    static File card = new File("card.WAV");
    ...
    // method to display human wins
    static void humanWinsMessage(CardTable ct)
    {
      JOptionPane.showMessageDialog(ct, "Player 1 Wins");
      System.exit(0);
    }
    // method to display computer wins
    static void computerWinsMessage(CardTable ct)
    {
      JOptionPane.showMessageDialog(ct, "Player 2 Wins");
      System.exit(0);
    }
    // method to play card sound
    static void playSound()
    {
      try
      {
        Clip clip = AudioSystem.getClip();
        clip.open(AudioSystem.getAudioInputStream(card));
        clip.start();     
      }
      catch (Exception e)
      {
    ...
    for (int k = 0; k < NUM_CARDS_PER_HAND; k++)
    {
      final int index = k;
        ...
        { 
          ...
          humanCardValue = 1000;
          // decrease the number of cards in hand
          humanCardsLeft--;
          // if there are no more cards left display you win message
          if(humanCardsLeft == 0)
            humanWinsMessage(ct);
        }
   ...
   // add events listeners (mouse listeners) to the cards in computers hand
   ...
   // method to add a new card in each of the three stacks
   ...
   // method that creates cannot play button and adds even listener to it
...
class CardTable extends JFrame 
{
   ...
   public CardTable(String title, int numCardsPerHand, int numPlayers) 
   {
      ...
      if((numCardsPerHand <= MAX_CARDS_PER_HAND) && (numPlayers <= MAX_PLAYERS))
      {
        ...
        pnlHumanHand.setBorder(new TitledBorder(new EmptyBorder(10, 10, 10, 10),"Player 1 Hand",TitledBorder.LEFT,TitledBorder.TOP,null,Color.WHITE));            
        c.gridx = 0;
        c.gridy = 1;
        add(pnlHumanHand, c);
        ...
        pnlComputerHand.setBorder(new TitledBorder(new EmptyBorder(10, 10, 10, 10),"Player 2 Hand",TitledBorder.LEFT,TitledBorder.TOP,null,Color.WHITE));
